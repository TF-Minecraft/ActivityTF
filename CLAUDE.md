...
- The three system-scope dependencies in `libs/` live as release assets on the private repo `JustinasLa/tfmc-deps` (tag `v1`). CI fetches them with the `DEPS_TOKEN` secret. If a dependency jar changes, tell the user to upload the new asset there and to regenerate `libs/SHA256SUMS`; do not commit the jar here.
...
- Every git and GitHub action runs as `JustinasLa`, the user's main account — never as `JustinasLaunikonis`, the university account that owns the repo. Before you commit or open a PR, check that `git config user.name` prints `JustinasLa` and that `gh auth status` shows `JustinasLa` as the active account; run `gh auth switch -u JustinasLa` if it does not. `JustinasLa` holds WRITE on the repo, so it can push branches and open PRs even though it is not the owner.
- Local builds land in `target/activity-1.0.jar` only. Nothing is copied to the project root any more; if a local deploy step used to read the root copy, point it at `target/`.
